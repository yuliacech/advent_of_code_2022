...
const solve = (filename) => {
    readFile((line) => {
        ...
        if ((leftStart >= rightStart && leftStart <= rightEnd) || (leftEnd >= rightStart && leftEnd <= rightEnd) || (rightStart >= leftStart && rightStart <= leftEnd) || (rightEnd >= leftStart && rightEnd <= leftEnd)) {
            pairCount++;
        }
        }
